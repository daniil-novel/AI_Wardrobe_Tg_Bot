...
def test_compose_can_start_without_required_env_file() -> None:
    ...
    assert "rabbitmq:3.13-management-alpine" in compose
    assert "rabbitmq-diagnostics" in compose
    assert "amqp://ai_wardrobe:change-me@rabbitmq:5672//" in compose
...
def test_env_example_exposes_port_overrides_without_secrets() -> None:
    ...
    for key in ("API_HOST_PORT", "MINIAPP_HOST_PORT", "POSTGRES_HOST_PORT", "RABBITMQ_HOST_PORT"):
        assert key in env_example
    assert "INTERNAL_API_URL=http://api:8000" in env_example
    assert "CELERY_BROKER_URL=amqp://ai_wardrobe:change-me@rabbitmq:5672//" in env_example
